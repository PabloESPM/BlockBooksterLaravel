...
@auth
    <div x-data="{ show: true }"
         x-show="show"
         @keydown.escape.window="show = false; setTimeout(() => $wire.closeFollowersModal(), 150)"
        ...
        <div @click.away="show = false; setTimeout(() => $wire.closeFollowersModal(), 150)"
            ...
            <button @click="show = false; setTimeout(() => $wire.closeFollowersModal(), 150)"
                    class="absolute top-4 right-4 text-2xl font-black hover:text-red-600 z-50">&times;</button>

            <h2 class="text-xl font-black uppercase mb-6 font-display border-b-2 border-black pb-2">Seguidores</h2>

            <div class="overflow-y-auto pr-2 custom-scrollbar flex-grow space-y-4">
                @forelse($users as $u)
                    <div wire:key="modal-follower-{{ $u->id }}">
                        <x-user-card
                            :user="$u"
