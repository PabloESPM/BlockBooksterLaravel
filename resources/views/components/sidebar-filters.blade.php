...
<aside class="w-full lg:w-72 flex-shrink-0 hidden lg:block">
    <x-card class="sticky top-24 space-y-8 bg-white" class="p-6">
        <form action="{{ route('books.index') }}" method="GET">
            ...
            <!-- Conservar parámetros de búsqueda de Búsqueda Avanzada -->
            ...
            <div class="mb-6">
                ...
                <select name="sort" class="neo-input w-full text-sm">
                    ...
                    <option value="oldest" {{ request('sort') == 'oldest' ? 'selected' : '' }}>Más antiguos primero</option>
            ...
            <div class="mb-6">
                ...
                <select name="language" class="neo-input w-full text-sm">
                    ...
                    <option value="fr" {{ request('language') == 'fr' ? 'selected' : '' }}>Francés</option>
                    <option value="de" {{ request('language') == 'de' ? 'selected' : '' }}>Alemán</option>
                </select>
            </div>
            ...
            <div class="mb-6">
                ...
                <div class="flex gap-2">
                    <input type="number" name="pages_from" value="{{ request('pages_from') }}" placeholder="Mín"
                           class="neo-input w-full text-sm px-2">
                    <input type="number" name="pages_to" value="{{ request('pages_to') }}" placeholder="Máx"
                           class="neo-input w-full text-sm px-2">
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <div class="flex gap-2">
                    <input type="number" name="year_from" value="{{ request('year_from') }}" placeholder="Desde"
                           class="neo-input w-full text-sm px-2">
                    <input type="number" name="year_to" value="{{ request('year_to') }}" placeholder="Hasta"
                           class="neo-input w-full text-sm px-2">
                </div>
            </div>
            ...
            <button type="submit" class="neo-btn-primary w-full text-center text-sm mb-4">Aplicar filtros</button>
            <a href="{{ route('books.index') }}" class="neo-btn-secondary w-full block text-center text-sm">Restablecer filtros</a>
