...
@section('title', $list->name . ' - Lista de Lectura')
                ...
                <form action="{{ route('dashboard.lists.update', $list) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description" id="edit_description" rows="3"
                                  class="w-full border-2 border-black p-2 focus:outline-none focus:shadow-[4px_4px_0px_#000] transition-shadow">{{ $list->description }}</textarea>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <select name="visibility" id="edit_visibility"
                            ...
                            <option value="public" {{ $list->visibility === 'public' ? 'selected' : '' }}>Pública (Visible para todos)</option>
                            <option value="friends" {{ $list->visibility === 'friends' ? 'selected' : '' }}>Solo Amigos</option>
                            <option value="private" {{ $list->visibility === 'private' ? 'selected' : '' }}>Privada (Solo yo)</option>
                        </select>
                    </div>

                    <button type="submit" class="w-full neo-btn-primary py-3 uppercase font-black">Guardar Cambios</button>
                </form>
            </div>
        </div>
    </div>
