...
@section('content')
    ...
    <div class="mb-8 border-b-4 border-black pb-4">
        <h1 class="text-4xl md:text-5xl font-black uppercase font-display">
            Resultados de búsqueda para "<span class="text-brand-blue">{{ $query }}</span>"
        </h1>
        <p class="text-lg font-bold text-gray-600 mt-2 uppercase">{{ $totalResults }} {{ Str::plural('resultado', $totalResults) }} encontrado{{ $totalResults !== 1 ? 's' : '' }}</p>
    </div>
    ...
    <form action="{{ route('search') }}" method="GET" class="mb-12 flex gap-3">
        ...
        <button type="submit" class="neo-btn-primary px-6">Buscar</button>
    </form>
                        ...
                        <x-card class="flex items-center gap-4 hover:-translate-y-1 transition-transform">
                            <a href="{{ route('authors.show', $author->id) }}" class="flex items-center gap-4 flex-1">
                                ...
                                <div class="flex-1">
                                    ...
                                    <p class="text-xs text-gray-500 font-bold">{{ $author->books_count }} {{ Str::plural('Libro', $author->books_count) }}</p>
                                </div>
                            </a>
                        </x-card>
                    @endforeach
                        ...
                        <x-card class="hover:-translate-y-1 transition-transform">
                            <a href="{{ route('lists.show', $list->id) }}">
                                <div class="flex items-center justify-between mb-4 pb-2 border-b-2 border-black/10">
                                    ...
                                    <span class="bg-green-100 text-green-800 text-xs font-bold uppercase px-2 py-0.5 border border-black flex-shrink-0 ml-2">Pública</span>
                                </div>
                                <div class="flex items-center gap-2 mb-3">
                                    ...
                                    <span class="text-xs font-bold text-gray-500 uppercase">{{ $list->user->name ?? 'Usuario' }}</span>
                                </div>
                                <div class="flex justify-between items-center text-xs font-bold text-gray-500 uppercase">
                                    <span>{{ $list->books_count }} {{ Str::plural('Libro', $list->books_count) }}</span>
