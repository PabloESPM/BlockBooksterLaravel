<?php
/**
 * Componente Livewire SFC — Tarjeta de Libro
 *
 * Componente puramente presentacional que recibe todos los datos
 * ya calculados desde el componente padre. No ejecuta consultas SQL propias.
 */

use Livewire\Component;

new class extends Component {
    // Identificador del libro (ISBN)
    public string $isbn = '';
...
<div x-data
    ...
    <a href="{{ route('books.show', $isbn) }}" wire:navigate class="absolute inset-0 z-10 focus:outline-none"
        aria-label="Ver {{ $title }}"></a>
    ...
    <div class="p-3 flex flex-col flex-grow">
        ...
        <div class="mt-auto pt-2 border-t-2 border-black/10 flex justify-between items-center relative z-20">
            @auth
                <button @click.prevent="Livewire.dispatch('open-add-to-list-modal', { bookId: '{{ $isbn }}' })"
            ...
            @endauth
            @auth
                <button @click.prevent="Livewire.dispatch('open-add-review-modal', { bookId: '{{ $isbn }}' })"
