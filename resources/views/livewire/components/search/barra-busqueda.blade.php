...
new class extends Component {
    ...
    {
        if (mb_strlen(trim($this->termino)) < 2) return collect();
...
<div class="relative" x-data>
    ...
    <div class="{{ $grande ? 'max-w-3xl mx-auto flex gap-3' : 'flex gap-2' }}">
        <div class="flex-1 relative">
            <input
                ...
                placeholder="Buscar libros, autores, usuarios, géneros, ISBN..."
                autocomplete="off"
            >
            {{-- Icono lupa --}}
            ...
            <div wire:loading wire:target="termino"
    ...
    @if($mostrarSugerencias && $this->haySugerencias)
        <div
            class="{{ $grande ? 'max-w-3xl mx-auto' : '' }} absolute top-full left-0 right-0 mt-1 bg-white border-2 border-black shadow-[4px_4px_0px_#000] z-50 max-h-[400px] overflow-y-auto"
            ...
            @if($this->sugerenciasGeneros->isNotEmpty())
                <div>
                    ...
                    @foreach($this->sugerenciasGeneros as $genero)
                        <a href="{{ route('books.index', ['genre' => $genero->id]) }}" wire:navigate wire:key="sug-gen-{{ $genero->id }}"
                           class="flex items-center gap-3 px-4 py-3 hover:bg-brand-yellow/30 transition-colors border-b border-gray-100 last:border-0">
