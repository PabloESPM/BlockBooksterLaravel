...
new #[Layout('layouts.admin')] #[Title('Panel de Control')] class extends Component {
    public function with()
    {
        return [
            'totalBooks' => Book::count(),
            'totalUsers' => User::count(),
            'pendingReviews' => Review::count(),
            'reportedLists' => FavList::count(),
        ];
    }
};
?>
...
<div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        <div class="bg-white border-2 border-black p-6 shadow-[4px_4px_0px_#000]">
            ...
            <div class="text-4xl font-black">{{ number_format($totalBooks) }}</div>
            <div class="text-xs font-bold text-green-600 mt-2">En base de datos</div>
        </div>
        <div class="bg-white border-2 border-black p-6 shadow-[4px_4px_0px_#000]">
            <div class="text-xs font-bold uppercase text-gray-500 mb-1">Total de Usuarios</div>
            <div class="text-4xl font-black">{{ number_format($totalUsers) }}</div>
            <div class="text-xs font-bold text-green-600 mt-2">Registrados en plataforma</div>
        </div>
        <div class="bg-white border-2 border-black p-6 shadow-[4px_4px_0px_#000]">
            <div class="text-xs font-bold uppercase text-gray-500 mb-1">Reseñas Creadas</div>
            <div class="text-4xl font-black text-brand-yellow">{{ number_format($pendingReviews) }}</div>
            <div class="text-xs font-bold text-gray-500 mt-2">Por moderar / general</div>
        </div>
        <div class="bg-white border-2 border-black p-6 shadow-[4px_4px_0px_#000]">
            <div class="text-xs font-bold uppercase text-gray-500 mb-1">Listas de Usuarios</div>
            <div class="text-4xl font-black text-red-500">{{ number_format($reportedLists) }}</div>
            <div class="text-xs font-bold text-gray-500 mt-2">Actividad de la comunidad</div>
        </div>
    </div>
        ...
        <div>
            ...
            <div class="bg-white border-2 border-black p-0">
                <div class="p-4 border-b border-gray-200 hover:bg-gray-50 flex justify-between items-center">
                    <div>
                        <span class="font-bold uppercase text-xs bg-blue-100 text-blue-800 px-2 py-0.5 mr-2 border border-blue-200">Sistema</span>
                        <span class="text-sm font-medium">Dashboard integrado con datos reales mediante Livewire.</span>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <a href="{{ route('admin.users.index') }}"
                   class="flex flex-col items-center justify-center p-6 bg-white border-2 border-black shadow-[4px_4px_0px_#000] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_#000] transition-all">
                    ...
                    <span class="font-bold uppercase">Ver Usuarios</span>
                </a>
                ...
                <a href="#" class="flex flex-col items-center justify-center p-6 bg-white border-2 border-black shadow-[4px_4px_0px_#000] hover:translate-y-[-2px] hover:shadow-[6px_6px_0px_#000] transition-all">
                    <svg class="w-8 h-8 mb-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
